...
| `segno_led_diffuser` | 6 | **White PLA** | Pill lens on a 3 mm shoulder, pushes into the faceplate slot from inside; **the shoulder is GLUED to the faceplate underside**, so keep that face clean. 66 × 12 × 5.85. **Print LENS-DOWN** — the lens face is then the bed face, with no layer lines across the light. A **one-LED segment cut from 60 LEDs/m bare WS2812B strip** snaps into a channel on the back (owner call 2026-08-28). **Bow the segment slightly, drop it past the lips, let it flatten** — its own adhesive is useless here, since the LEDs must face the lens, which turns the tape to face away into the console. The strip is the spring; nothing printed flexes. Grip is 0.4 mm per edge; both ends open for the 3 wires. One per *mappable* pedal — TRACK1-4, CLEAR, BANK. REC/PLAY, STOP, UNDO and MODE have no LED: they are fixed transport, so there is nothing to indicate. |
...
| LED puck (single WS2812B) | `led_strip/segno_led_strip_gerbers.zip` | 0 | **NOT ORDERED for the console.** Owner call 2026-08-28: the indicators are one-LED segments cut from a 60 LEDs/m bare strip, and the diffuser channel is sized for that (10 mm wide, 0.35 thick), not for this 16×8 board. The design is kept because it is finished and the footprint may suit another build — but ordering it will not fit the current diffuser. |

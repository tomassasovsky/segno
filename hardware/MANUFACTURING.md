...
| `segno_pedal_tile_*_white` + `_cover` | 10 pairs | **White + Black resin** | Pedal name tiles, **RESIN, two parts** (#931). `_white` is the WHOLE tile — trapezoid 54.36 (back) / 53.76 (toe) × 19.90 × 2.20, letters standing 0.4 proud — printed entirely in WHITE. `_cover` is the black 0.4 letter layer that drops over those letters and sits flush at 2.20; total is unchanged. ⚠️ **The assembled face is FLAT** — letters and field both finish at 2.20. The FDM tile deliberately sat its black field 0.4 below the pad, out of the scuff line; **neither resin route keeps that**, so it is not what separates them (see `TILE_TEXT_T`, open call #931). **Fit the WIDE edge toward the cable end** — it carries the top of the glyphs, so the wrong way round reads upside down. 0.08 mm clearance per side; the gap shows WHITE, which reads as a slightly bolder letter rather than a dark halo. ⚠️ **The cover is not always one piece**: the counters in B/A/R/O/D/4 are islands. REC/PLAY, STOP and TRACK1–3 are 1 piece; TRACK4 is 2; UNDO, MODE and CLEAR are 3; BANK is 4 — **10 loose islands over the ten tiles**. They self-locate in their counters, but count them out before you start gluing. Only `_white` and `_cover` ride in `segno_3dprint.zip`: the plain `segno_pedal_tile_<LABEL>.step` is the two-body **appearance reference** the Fusion doc reads its colours off, not a print file, and there is no plain `.stl` any more — that was the single-extruder filament-change mesh #931 retired. Text is generated from the same `PEDALS`/`SILK_SYMBOLS` source as the faceplate legends, so REC/PLAY and STOP carry the dot+plus+triangle and square rather than words. |
| `segno_pedal_tile_*_inlay` | 0 (alternative) | **Black resin + white fill** | The OTHER route, shipped for comparison until one is chosen (#931). ONE black part, 2.2 solid, letters as 0.4-deep pockets: fill with white resin, scrape flush, cure. No second print, no loose islands, no clearance gap — at the cost of scraping a resin fill flush by hand. It finishes flush, but so does `_cover`, so the choice is islands-and-a-second-print versus a hand fill. Print one of each and pick; the loser gets deleted. Deliberately **not** in `segno_3dprint.zip` — quantity 0, and shipping it invites a quote for a part nobody has chosen (same reason as `segno_screen7_fit_test`). |

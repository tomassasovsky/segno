...
Material: **2.0 mm 1050 aluminium** (the shop's stock; softer than 5052 and folds R2 without complaint), K-factor 0.33, R2 tooling (bend notes
...
| `segno_platform_mid` | 2 | **BLACK** PETG/ASA, ≥40% infill | Tall CLEAR/BANK tub (walls to 73.7 — row 2 sits on the 16in aperture's front edge since #796, 37.6 mm forward of the #366 label-top position; deck raised for flush-at-rim seating #373), a **solid block** under the deck — no modelled cavity, the slicer's infill does the hollowing, so ≥40% here means ~410 cm³ of envelope and roughly 250 g of filament per part — standard **M3 5×5 inserts** (4 per pedestal) + the same pocket, channels, notch and perimeter relief. |
...
| `segno_ring_diffuser` | 1 | **White PLA** | Annular lens for the encoder LED ring. Cut for the **Ring 24** (Ø67 window); its back-plate radii are hardcoded, not derived — re-derive them with any window resize. The back plate is OPEN over the LED circle (r 25.6–33.2): the PR #990 board carries the Ring 24 on a 2.54 mm pin strip, and a web there collided with it; the ring glows through the 2.4 mm lens. |

...
| `segno_floor_rail_front` | 8 | **BLACK** PETG/ASA, ≥40% infill | Front floor rails (issue #1019), 199.29 mm, four screws. **Every segment of a rail is the same part** — eight of this one, four of the rear, two of the mid — because the rail span comes from the pedal pitch, so each segment is exactly two pedals wide and the screw pattern repeats. **Print flat, channel side up**: the channel needs no support and the floor face is the bed face. Each carries a 25.4 × 3.2 mm self-adhesive SOLID neoprene strip (LSGCQ 1" × 1/8") pressed into its 25.2 × 1.5 mm channel — 0.2 mm under size on purpose, so the walls hold it and the adhesive is not in the load path. **Not grit tape**: this gets dragged across floors. The strip stands 1.7 mm proud and is the only thing touching the ground. The channel ends are rounded on a 12.6 mm radius, and a square-cut strip stops dead at the tangent — **round the two corners with the same scissors** as you cut it, or you lose 12.6 mm of contact at every end. **Fit the rails first, then press the strip in** — the M3 heads sink into counterbores inside the PETG, above the channel roof, so the strip runs over them unbroken and never needs punching. 2,765 mm of strip for the set: buy the 20 ft roll. |
| `segno_floor_rail_rear` | 4 | as above | Rear floor rails, 199.29 mm, two screws 63.5 and 109.5 mm from the low-u end — **not** a symmetric pair, so all four go on the same way round. The rail sits at v 343.25, **not** where the rubber feet were: the buck converters bolt through v 367.5 with a floor-side washer and nut, so hardware protrudes there. Clearance to the screen-stand pilots is 1.5 mm each side — check it on the first set. |
| `segno_floor_rail_mid` | 2 | as above | CLEAR/BANK floor rails, 186.89 mm, four screws. |

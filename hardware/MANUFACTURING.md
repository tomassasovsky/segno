...
| `segno_led_diffuser` | 6 | **White PLA** | Pill lens on a 3 mm shoulder, pushes into the faceplate slot from inside; **the shoulder is GLUED to the faceplate underside**, so keep that face clean. 66 × 12 × 6.9. **Print LENS-DOWN** — the lens face is then the bed face, with no layer lines across the light. The LED puck (16 × 8 × 1.6, `hardware/led_strip/`) fits a slot on the back: **tilt one long edge under its lip, then press the other past the lead-in** — it will not go in square. Lips engage 0.25 mm per side, a retention detent rather than a structural snap (the rails are only 2.5 mm deep and cannot flex further). Both short ends are open for the 3-wire daisy chain. One per *mappable* pedal — TRACK1-4, CLEAR, BANK. REC/PLAY, STOP, UNDO and MODE have no LED: they are fixed transport, so there is nothing to indicate. |

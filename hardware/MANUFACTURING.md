...
| `segno_floor_rail_*` | 17 | **BLACK** PETG/ASA, ≥40% infill | The console's floor supports (issue #1019). Five rails in 17 bed-sized segments: `front_a`/`front_b` and `rear` in five each, `mid_a`/`mid_b` in one. **Print flat, channel side up**, so the channel needs no support and the floor face is the bed face. Each carries a 25.4 × 3.2 mm self-adhesive SOLID neoprene strip (LSGCQ 1" × 1/8") pressed into its 25.2 × 1.5 mm channel — 0.2 mm under size on purpose, so the walls hold it and the adhesive is not in the load path. **Not grit tape**: this gets dragged across floors. The strip stands 1.7 mm proud and is the only thing touching the ground. **Fit the rails first, then press the strip in** — the M3 heads sink into counterbores inside the PETG, above the channel roof, so the strip runs over them unbroken and never needs punching. Joints land on screw stations, so one screw clamps two segment ends; a segment on one screw would pivot. 2,819 mm of strip for the set, so buy the 20 ft roll. |

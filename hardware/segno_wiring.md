...
| Neutrik NJ6FD-V ×2 (`CTRL_1`/`CTRL_2`) | 6-pole switching 1/4" jack, rear-mounted through a Ø12 hole with its snap cap (needs the 1.5 mm panel). Vertical PCB pins, no lugs: solder the leads to T / R / S, leave TN / RN / SN open. Expression pedal OR footswitch on the same jack, auto-detected (tip → ADC with pull-up, ring → 3V3 through 1 k) A two-switch pedal on one TRS plug (BOSS FS-6 A&B) puts its B switch on the ring: on board v2 run one wire from each jack's ring pin (J20/J21 pin 2) to J22's GP20/GP21 pads and firmware ≥ 1.1 reads it. Without the wire, use the pedal's separate A and B mono jacks, one per CTRL |

...
static void test_processor_uid_unchanged() {
  ...
  CHECK(std::memcmp(segno_vst3_reverb::kProcessorUID, expected, 16) == 0);
}
...
static void test_controller_uid_unchanged() {
  ...
  CHECK(std::memcmp(segno_vst3_reverb::kControllerUID, expected, 16) == 0);
}

static void test_processor_and_controller_uid_distinct() {
  std::printf("test_processor_and_controller_uid_distinct\n");
  CHECK(std::memcmp(segno_vst3_reverb::kProcessorUID, segno_vst3_reverb::kControllerUID,
                     16) != 0);
}

...
static void test_processor_uid_unchanged() {
  ...
  CHECK(std::memcmp(segno_vst3_drive::kProcessorUID, expected, 16) == 0);
}
...
static void test_controller_uid_unchanged() {
  ...
  CHECK(std::memcmp(segno_vst3_drive::kControllerUID, expected, 16) == 0);
}

static void test_processor_and_controller_uid_distinct() {
  std::printf("test_processor_and_controller_uid_distinct\n");
  CHECK(std::memcmp(segno_vst3_drive::kProcessorUID, segno_vst3_drive::kControllerUID,
                     16) != 0);
}

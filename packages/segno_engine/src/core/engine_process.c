 ...
 * this is the audio-thread re-check for the block in between. */
static int le_looper_mode_switch_blocked(le_engine* e) {
  if (e->count_in_total > 0) return 1;
  for (int32_t t = 0; t < e->track_count; ++t) {
 ...
 * and one callback carrying a few hundred microseconds of memset is the
...
static void apply_command(le_engine* e, const le_command* cmd, uint64_t frame) {
  switch (cmd->code) {
    ...
    /* ---- looper mode (B2a, D4; see le_looper_mode_switch_blocked above). Not
    ...
    case LE_CMD_SET_LOOPER_MODE: {
      ...
      if (le_looper_mode_switch_blocked(e)) break;
    ...
    case LE_CMD_SET_LANE_PAN: {
      ...
      le_store_pan(&pl->a_pan_bits, &pl->a_pan_gl_bits, &pl->a_pan_gr_bits,
                   cmd->lanef.value);
      break;
    }
...
static inline void mix_tracks_frame(
    ...
    float trk_fx_params[][LE_FX_MAX][LE_FX_PARAMS],
    int32_t trk_fx_enabled[][LE_FX_MAX], int at_has_fx, int32_t at_fx_count,
...
void le_engine_process(le_engine* e, float* output, const float* input,
  ...
  for (uint32_t f = 0; f < frames; ++f) {
    ...
    mix_tracks_frame(e, in_c, out, f, ch_in, ch_out, tc, sr, fx_cap, excluded,
                     ...
                     trk_fx_enabled, at_has_fx, at_fx_count, at_fx_type,
    ...
    if (perf_follow) perf_tap_master_frame(e, out, f, ch_out);

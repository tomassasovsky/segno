...
void main() {
  ...
  group('spanish param names', () {
    ...
    testWidgets('render whole in the tile', (tester) async {
      // 1.5 as well as 1.0: the shrink-to-fit cap scales with the text, so
      // the set of names that fit whole must be the same set at a raised
      // system text scale — and the name row must never grow past its
      // one-line height and squeeze the value box into overflowing.
      for (final scale in [1.0, 1.5]) {
        ...
        for (final (i, name) in names.indexed) {
          ...
          expect(
            ...
            isFalse,
            reason:
                '"$name" must not truncate at the tile width '
                '(scale $scale)',
          );
          expect(
            tester.getSize(find.byKey(Key('tile_$i'))).height,
            FxParamTileMetrics.height,
            reason: 'the grid stays a grid under "$name" (scale $scale)',
          );
        }
      }
    });

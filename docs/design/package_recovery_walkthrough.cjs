...
const dir=path.join(__dirname,'recovery-expansion-previews','walkthroughs');
const clips=JSON.parse(fs.readFileSync(path.join(dir,'clips.json'),'utf8'));
if(clips.length!==6)throw Error('Six complete clips are required.');
...
console.log('Assembled '+Math.round(cursor)+' seconds with six chapters.');

...
- [ ] Confirm LED feedback: standby ring breathes green; once recording or
      playing, the ring is green with a rec-red / mute-green playhead
      (including the first take, before a loop length exists); per-track LEDs;
      goodbye-frame on shutdown.

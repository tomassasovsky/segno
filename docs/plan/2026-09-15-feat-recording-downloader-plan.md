...
4. Verify failure cases automatically and connect/download on the real appliance.
...
- Preview prepares, plays and seeks through a real appliance recording; closing removes its temporary copy | verify: manual click a file's Play button, pause, seek, resume and close; inspect the download folder
...
The owner added full recording previews with seeking. Add a per-file preview
button and AVFoundation playback controls. Preparation uses the existing verified
download pipeline into a private temporary folder. Preview close, replacement,
cancellation and app exit clean up that folder. Preparing a large preview requires
transferring the whole file first; it is not live streaming.

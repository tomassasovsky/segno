  ...
  Widget build(BuildContext context) => LayoutBuilder(
    builder: (context, constraints) {
      assert(
        ...
        'a face is drawn INTO a box — give it one, with widthFor()',
      );
      // Per axis, because the box need not carry the art's aspect: one scale
      // for both would slide the nameplate off the plate the moment it did
      // not.
      final scaleX = constraints.maxWidth / artSize.width;
      final scaleY = constraints.maxHeight / artSize.height;
      return CustomPaint(
        ...
        child: legend == null
            ...
            : Stack(
                children: [
                  Positioned(
                    left: namePlate.left * scaleX,
                    top: namePlate.top * scaleY,
                    width: namePlate.width * scaleX,
                    height: namePlate.height * scaleY,
...
class PedalFacePainter extends CustomPainter {
  ...
  void _paintGrips(Canvas canvas) {
    ...
    final edge = Paint()
      ...
      ..style = PaintingStyle.stroke
      ..strokeWidth = 0.15;
    // Every grip is the same size, so ONE shader is built and the canvas moves
    // under it. Thirty shaders per face times ten faces is work the picture
    // does not need doing.
    final lit = Paint()
      ..shader =
          const LinearGradient(
            ...
            Rect.fromCircle(center: Offset.zero, radius: _gripTopRadius),
          );
  ...
  //
  // Built once: they never vary, and a face that rebuilt seven paths on every
  // paint would do it ten times over on one screen.
  // ---------------------------------------------------------------------------
